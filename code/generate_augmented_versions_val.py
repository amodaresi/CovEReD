...
train_ds = load_dataset("json", data_files=os.path.join(DATA_FILEPATH, f"train_entFix.json"), keep_in_memory=True)["train"]
print(ds)

# identifying relations of each vertex
relation_maps = []
for i in range(len(train_ds)):
    relation_map = {v: set() for v in range(len(train_ds[i]["vertexSet"]))}
    for relation in train_ds[i]["labels"]:
...
WINDOW = 16
for i in range(len(train_ds)):
    vertexSet = train_ds[i]["vertexSet"]
    all_tokens = [s for sent in train_ds[i]["sents"] for s in sent]
    for vertex_id, vertex in enumerate(vertexSet):

        newVertex = {
            "aliases": set([s["name"] for s in vertex]),
            "contexts": set([" ".join(all_tokens[max(s["global_pos"][0]-WINDOW,0):min(s["global_pos"][0]+s["pos"][1]-s["pos"][0]+WINDOW, len(all_tokens))]) for s in vertex]),
...
def get_alternatives(candidate, SELFSIM=T_SELFSIM, CONTEXTSIM=T_CONTEXTSIM, SELFSIM_UPPER=T_SELFSIM_UPPER):
    alternatives = []
...
NO_of_BUCKETs = 10
for i in tqdm(range(NO_of_BUCKETs)):
    bucket = []
